...
const ProductDetails = () => {
  ...
  if (!product)
    return <div className="min-h-screen flex justify-center items-center text-white bg-gray-900">Loading product details...</div>;
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-900 via-gray-950 to-black text-white p-6 flex flex-col md:flex-row gap-2">
      ...
      <div className="flex-1 flex flex-col items-center">
        <div className="w-full max-w-lg border border-gray-700 rounded-lg overflow-hidden">
          <img src={mainImage || "/images/default.png"} alt={product.title} className="w-full h-[400px] object-cover" />
        </div>
      ...
      <div className="flex-1 flex flex-col gap-6">
        ...
        <p className="text-sm text-gray-300">
          <div className={`border-2 w-fit py-1 px-2 rounded-full`}>
            <p className={`text-sm ${product.isNegotiable ? "text-green-500" : "text-red-400"}`}>
        ...
        <button className="mt-4 bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg font-semibold transition w-fit" onClick={handleChatPart}>
          💬 Chat with Seller
        </button>
      </div>
    </div>
