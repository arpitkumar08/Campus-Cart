...
const UserRow = ({ report }) => {
  ...
  return (
    <tr className="border-b border-white/20 hover:bg-zinc-800/60 transition-colors">
      ...
      <td className="py-3 px-4">{report.reportedUser?.email || "N/A"}</td>
    ...
    <div className="text-white px-6 py-6">
      ...
      <div className="border border-gray-50 rounded-xl p-4 bg-slate-900/50">
        {loading ? (
          ...
          <p className="text-gray-400">No reported users found.</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-gray-300 border-collapse">
              ...
              <tbody>
                {reports.map((report) => (
                  <UserRow key={report._id} report={report} />
                ))}
              </tbody>
            </table>
          </div>
